...
System_Arms = datetime.time(19, 00)
System_Disarms = datetime.time(06, 00)
#if (System_Arms <= Current_Time <= System_Disarms): #Keep commented for presentation
    #override_flag = True
...
class Matter(object):
    ...
    def on_enter_Verification(self):
        ...
        ##Here Ident_Flag=1 is fail, Ident_Flag=0 means pass.##
        #global Ident_Flag
        #if (Ident_Flag): output_msg = 'Face not detected or unauthorized driver, system remains locked.'
...
if (Alc_Flag): SBS.Measurement_Failed()
else: SBS.Measurement_Successful()
    #Measurement completed, go to facial verification via 'Verification'
SBS.Verification_Successful()
    #Both verification methods successful, go to 'Passed'
SBS.Verification_Failed()
SBS.SMS_Sent()
#if (Ident_Flag): SBS.Measurement_Failed()
#else: SBS.Verification_Successful()
GPIO.cleanup()
print ('GPIO reset')
print (override_flag)
